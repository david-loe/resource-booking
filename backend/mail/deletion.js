...
function sendDeletionMail(deletedBooking, recipientName, recipientMail) {
    ...
    deletedBooking.startDateStr = new Date(deletedBooking.startDate).toLocaleDateString(process.env.VUE_APP_I18N_LOCALE, dateStringOptions)
    deletedBooking.endDateStr = new Date(deletedBooking.endDate).toLocaleDateString(process.env.VUE_APP_I18N_LOCALE, dateStringOptions)
    ...
    const plainText = i18n.t("mail.deletion.heading") + '\n\n' +
        ...
        i18n.t("labels.from") + ': ' + deletedBooking.startDateStr + '\n' +
        i18n.t("labels.to") + ': ' + deletedBooking.endDateStr + '\n' +
    ...
    mailClient.sendMail({
        ...
        icalEvent: helper.icalEventForEmailAttachments(deletedBooking, 'CANCEL', recipientMail, recipientName)
    })
}
